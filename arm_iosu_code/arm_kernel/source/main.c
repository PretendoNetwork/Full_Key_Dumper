...
int _main() {
	...
	*(uint32_t**)0x0012F000 = *(uint32_t**)0x1016AD18; // Always in the 0x08148xxx range
	...
	CRYPTO_KeyHandle *keyHandles = (CRYPTO_KeyHandle*)(0x0402bb2c + 0x4280000);
	CRYPTO_KeyData *keyData = (CRYPTO_KeyData*)(0x0402972c + 0x4280000);

	/* Dump the BOSS keys */
	memcpy_((void*)0x01FFF400, &keyData[keyHandles[0x1F].index].data, 16); 		// 0x1F: Key to encrypt/decrypt SpotPass data files
	memcpy_((void*)0x01FFF410, &keyData[keyHandles[0x27].index].data, 16); 		// 0x27: Key to encrypt/decrypt "pushmore" links
	memcpy_((void*)0x01FFF420, &keyData[keyHandles[0x20].index].data, 32); 		// 0x20: SpotPass data file HMAC key (first 32-bytes)
	memcpy_((void*)0x01FFF440, &keyData[keyHandles[0x20].index + 1].data, 32); 	// 0x26: SpotPass data file HMAC key (last 32-bytes)
